...
import { cppContent } from '../data/cppContent';
import { useAppContext } from '../context/AppContext';
import TrophyModal from '../components/TrophyModal';
import CodeChallenge from '../components/CodeChallenge';

const sectionIcons: Record<string, React.ReactNode> = {
  'cpp-fundamentals': <Terminal size={20} />,
  'oop': <LayoutTemplate size={20} />,
  'stl': <Library size={20} />,
  'memory-management': <MemoryStick size={20} />,
  'advanced-features': <BrainCircuit size={20} />,
  'modern-cpp': <Zap size={20} />,
  'concurrency': <Clock size={20} />,
  'io-file-handling': <Globe size={20} />,
};

export default function CppReference() {
  const { deductCoin, updateStreak, addCoins } = useAppContext();
  const [activeSectionId, setActiveSectionId] = useState(cppContent[0].id);
  ...
  const activeSection = cppContent.find(s => s.id === activeSectionId) || cppContent[0];
  ...
  const handleNextSection = () => {
    const currentIndex = cppContent.findIndex(s => s.id === activeSectionId);
    if (currentIndex < cppContent.length - 1) {
      setActiveSectionId(cppContent[currentIndex + 1].id);
      resetQuiz();
    }
  };
    ...
    <div className="flex h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 overflow-hidden font-sans">
      ...
      <aside className="w-72 border-r border-slate-200/50 dark:border-white/10 bg-white/80 dark:bg-card-dark/80 backdrop-blur-xl flex flex-col shrink-0 overflow-y-auto no-scrollbar hidden md:flex">
        <div className="p-6 border-b border-slate-200/50 dark:border-white/10 flex items-center gap-3">
          <div className="size-10 rounded-xl bg-accent-cpp flex items-center justify-center text-white shadow-lg shadow-accent-cpp/20">
            <BookOpen size={24} />
          </div>
          <div>
            <h1 className="font-bold text-lg leading-tight">C++ Mastery</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">Complete Reference</p>
          </div>
        </div>
        
        <nav className="p-4 space-y-1">
          {cppContent.map((section) => (
            <button
              key={section.id}
              onClick={() => {
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${
                activeSectionId === section.id 
                  ? 'bg-accent-cpp/10 text-accent-cpp font-semibold' 
                  : 'hover:bg-slate-100 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400'
              }`}
            >
              <span className={activeSectionId === section.id ? 'text-accent-cpp' : 'text-slate-400'}>
                {sectionIcons[section.id] || <Box size={20} />}
              </span>
        ...
        <div className="mt-auto p-6 border-t border-slate-200/50 dark:border-white/10">
          <Link to="/dashboard" className="flex items-center gap-2 text-sm text-slate-500 hover:text-accent-cpp transition-colors">
            <Home size={16} />
            Back to Dashboard
          </Link>
        </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-slate-50/50 dark:bg-[#050505]">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-10 no-scrollbar relative">
          ...
          <div className="hidden md:block absolute top-10 right-10 z-20">
            <Link 
              to="/dashboard" 
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/80 dark:bg-card-dark/80 backdrop-blur-md border border-slate-200/50 dark:border-white/10 text-slate-600 dark:text-slate-300 hover:text-accent-cpp hover:border-accent-cpp/50 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"
            >
              <Home size={18} />
            ...
            {!quizMode ? (
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-cpp/10 text-accent-cpp text-xs font-bold uppercase tracking-wider">
                    {sectionIcons[activeSectionId] || <Box size={20} />}
                    Section {cppContent.findIndex(s => s.id === activeSectionId) + 1}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative group">
                    <div className="absolute -inset-1 bg-gradient-to-r from-accent-cpp to-blue-500 rounded-2xl blur opacity-10 group-hover:opacity-20 transition duration-1000"></div>
                ...
                {/* @ts-ignore - activeSection might not have challenge property but we check it */}
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-6">
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-accent-cpp hover:bg-accent-cpp/90 text-white font-bold py-4 px-8 rounded-2xl transition-all shadow-xl shadow-accent-cpp/20 hover:shadow-accent-cpp/40 hover:-translate-y-0.5 group"
                  >
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      disabled={cppContent.findIndex(s => s.id === activeSectionId) === 0}
                    ...
                    <button 
                      ...
                      disabled={cppContent.findIndex(s => s.id === activeSectionId) === cppContent.length - 1}
                      className="size-14 flex items-center justify-center bg-white/80 dark:bg-card-dark/80 backdrop-blur-md border border-slate-200/50 dark:border-white/10 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/5 disabled:opacity-30 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"
                    >
              ...
              >
                {!quizFinished ? (
                  <div className="space-y-8">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-end">
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] font-bold text-accent-cpp uppercase tracking-tighter">Each question costs 1 coin</p>
                        </div>
                        <span className="text-accent-cpp font-black text-xl">{score}/{activeSection.quizzes.length}</span>
                      </div>
                      <div className="h-2 w-full bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden">
                        <motion.div 
                          className="h-full bg-accent-cpp"
                          initial={{ width: 0 }}
                    ...
                    <div className="grid gap-3">
                      {activeSection.quizzes[currentQuizIndex].options.map((option, idx) => {
                        ...
                        let bgColor = "bg-white/80 dark:bg-card-dark/80 backdrop-blur-md hover:border-accent-cpp/50";
                  ...
                  >
                    <div className="size-32 bg-accent-cpp/10 rounded-full flex items-center justify-center mx-auto text-accent-cpp">
                      <Trophy size={64} />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <button 
                        ...
                        className="flex-1 flex items-center justify-center gap-2 bg-accent-cpp hover:bg-accent-cpp/90 text-white font-bold py-4 px-8 rounded-2xl transition-all shadow-lg shadow-accent-cpp/20 hover:shadow-xl hover:shadow-accent-cpp/40 hover:-translate-y-0.5"
                      >
                        Next Section
          ...
          {cppContent.map((section) => (
            <button
              ...
              className={`shrink-0 size-12 rounded-xl flex items-center justify-center transition-all ${
                activeSectionId === section.id 
                  ? 'bg-accent-cpp text-white shadow-lg shadow-accent-cpp/20' 
