...
const sectionIcons: Record<string, React.ReactNode> = {
  ...
  'advanced-types': <Box size={20} />,
  'oop': <LayoutTemplate size={20} />,
  'generics': <Zap size={20} />,
  'functions': <FunctionSquare size={20} />,
  'type-manipulation': <BrainCircuit size={20} />,
  'namespaces-modules': <Globe size={20} />,
  'decorators': <AlertTriangle size={20} />,
  'compiler-config': <Wrench size={20} />,
  'async-programming': <Clock size={20} />,
};

export default function TsReference() {
  const { deductCoin, updateStreak, addCoins } = useAppContext();
  const [activeSectionId, setActiveSectionId] = useState(tsContent[0].id);
  ...
  const handleNextSection = () => {
    const currentIndex = tsContent.findIndex(s => s.id === activeSectionId);
    if (currentIndex < tsContent.length - 1) {
      setActiveSectionId(tsContent[currentIndex + 1].id);
      resetQuiz();
    }
  };
    ...
    <div className="flex h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 overflow-hidden font-sans">
      ...
      <aside className="w-72 border-r border-slate-200/50 dark:border-white/10 bg-white/80 dark:bg-card-dark/80 backdrop-blur-xl flex flex-col shrink-0 overflow-y-auto no-scrollbar hidden md:flex">
        <div className="p-6 border-b border-slate-200/50 dark:border-white/10 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-lg leading-tight">TS Mastery</h1>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${
                activeSectionId === section.id 
                  ? 'bg-accent-ts/10 text-accent-ts font-semibold' 
                  : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'
              }`}
            >
              <span className={activeSectionId === section.id ? 'text-accent-ts' : 'text-slate-400'}>
            ...
            {!quizMode ? (
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-ts/10 text-accent-ts text-xs font-bold uppercase tracking-wider">
                    {sectionIcons[activeSectionId]}
                    Section {tsContent.findIndex(s => s.id === activeSectionId) + 1}
                  </div>
              ...
              >
                {!quizFinished ? (
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-end">
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] font-bold text-accent-ts uppercase tracking-tighter">Each question costs 1 coin</p>
                        </div>
                        <span className="text-accent-ts font-black text-xl">{score}/{activeSection.quizzes.length}</span>
                      </div>
                      <div className="h-2 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                        <motion.div 
                          className="h-full bg-accent-ts"
                          initial={{ width: 0 }}
                  ...
                  >
                    <div className="size-32 bg-accent-ts/10 rounded-full flex items-center justify-center mx-auto text-accent-ts">
                      <Trophy size={64} />
                    </div>
                    <div className="space-y-2">
          ...
          {tsContent.map((section) => (
            <button
              ...
              className={`shrink-0 size-12 rounded-xl flex items-center justify-center transition-all ${
                activeSectionId === section.id 
                  ? 'bg-accent-ts text-white shadow-lg shadow-accent-ts/20' 
